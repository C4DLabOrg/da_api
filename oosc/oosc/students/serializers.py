...
class StudentsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Students
        fields = ('id','student_id','guardian_name','short_name','created','is_oosc','modified','guardian_phone','active','date_enrolled', 'emis_code','last_attendance','class_name','school_name','total_absents','student_name', 'fstname','midname','lstname','date_of_birth', 'admission_no', 'class_id',
...
class SimpleStudentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Students
        fields=('id','student_id','guardian_name','short_name','created','is_oosc','modified','active','class_id','guardian_phone','student_name','gender','date_enrolled', 'emis_code','last_attendance','total_absents', 'fstname','midname','lstname')
...
class ImportResultsSerializer(serializers.Serializer):
    ...
    total_dublicates=serializers.IntegerField()
    success_percentage=serializers.SerializerMethodField()
    def get_success_percentage(self,obj):
        total=obj.total_fails+obj.total_success+obj.total_dublicates
        if total ==0:
            return "0%"
